...
import 'package:q_check/Screens/spices/spices_details/ambahaldi.dart';
import 'package:q_check/Screens/spices/spices_details/haldi.dart';
import 'package:q_check/Screens/spices/spices_details/kalihaldi.dart';
import 'package:q_check/Screens/spices/spices_details/karchur.dart';
import 'package:tflite/tflite.dart';
import 'dart:io';

class ModelTestCurcumaLonga extends StatefulWidget {
  @override
  _ModelTestCurcumaLongaState createState() => _ModelTestCurcumaLongaState();
}

class _ModelTestCurcumaLongaState extends State<ModelTestCurcumaLonga> {
  bool _loading = false;
  File? _image;
  List? _output;
  ...
  classifyImage(File image) async {
    ...
    if (_output != null &&
      ...
      Future.delayed(Duration(milliseconds: 1000), () {
        Navigator.push(
            context, MaterialPageRoute(builder: (context) => CurcumaLonga()));
      });
    } else if (_output != null &&
        (_output![0]['label'].toString().substring(0, 1) == "1")) {
      Future.delayed(Duration(milliseconds: 1000), () {
        Navigator.push(
            context, MaterialPageRoute(builder: (context) => CurcumaAmada()));
      });
    } else if (_output != null &&
        (_output![0]['label'].toString().substring(0, 1) == "2")) {
      Future.delayed(Duration(milliseconds: 1000), () {
        Navigator.push(
            context, MaterialPageRoute(builder: (context) => CurcumaCasia()));
      });
    } else if (_output != null &&
        (_output![0]['label'].toString().substring(0, 1) == "3")) {
      Future.delayed(Duration(milliseconds: 1000), () {
        Navigator.push(context,
            MaterialPageRoute(builder: (context) => CurcumaZeoderia()));
      });
    } else {
      Future.delayed(Duration(milliseconds: 1000), () {
        _onAlertButtonPressed1(context);
  ...
  void loadModel() async {
    await Tflite.loadModel(
      model: "assets/models/model_curcumalonga.tflite",
      labels: "assets/labels/labelscurcumalonga.txt",
    );
  }
